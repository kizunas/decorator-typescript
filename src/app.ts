...
interface ValidatorConfig {
  // index型の構文を使用
  [prop: string]: {
    [validatableProp: string]: string[] // ['required', 'positive']のようなバリデータのリストを保存する
  }
}

const registeredValidators: ValidatorConfig = {}; // デコレータが実行されたときに初めて値が登録される。

function Required(target: any, propName: string) { // プロパティーに対するデコレータはプロパティティスクリプターを受け取らない
...
courseForm.addEventListener('submit', event => {
  event.preventDefault(); // http requestを行わないため
  const titleEl = document.getElementById('title') as HTMLInputElement;
  const priceEl = document.getElementById('price') as HTMLInputElement;

  const title = titleEl.value;
  const price = +priceEl.value; // plusをつけてnumberにキャストする。
